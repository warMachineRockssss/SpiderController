...
class LinkSpider(scrapy.Spider):
    ...
    def start_requests(self):
        # 热门视频只有一页2---
        url = 'http://www.bizhimarket.cn/vodshow/6--hits------1---.html'
    ...
    def parse(self, response):
        ...
        # 获取当前页面中的所有链接
        movie_item_list = rendered_response.css('.stui-vodlist__box')

        movie_info_list = []

        # 这里是解析列表页的所有信息，但是是不完整的，所以我们在下面请求具体页面
        for item in movie_item_list:
            _link = item.css('.stui-vodlist__thumb.lazyload::attr(href)').extract_first()
            ...
            try:
                movie_info_list.append({
                    ...
                    'movie_poster': item.css('.stui-vodlist__thumb.lazyload::attr(data-original)').extract_first()
                })
        ...
        for element in movie_info_list[:2]:
            if element.get('movie_addr') is not None:
                ...
                #     'url': "http://www.bizhimarket.cn/" + element.get('movie_addr').replace("m-", "play-") + "-1-1.html",
                # }
    ...
    def get_m3u8_link(self, response, movie_name, movie_actor, movie_poster):
        ...
        _rendered_page = get_renderer_page(url=re.sub(r'\?.*','', response.url))
        # 将渲染后的页面传递给 Scrapy 的 TextResponse 对象
